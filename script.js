...
async function initData() {
    ...
    _supabase.channel(`room_${ROOM}`)
        ...
        .subscribe((status) => {
            if(status === 'SUBSCRIBED') document.getElementById('online-status').classList.add('live');
        });
}
...
async function nukeRoom() {
    if(confirm("☢️ CUIDADO: Isso vai apagar TODOS os post-its desta sala. Confirmar?")) {
        state.forEach(col => col.cards = []);
        await save("SALA RESETADA POR " + user.name.toUpperCase());
        renderBoard();
    }
}
...
async function addCard(cid) {
    ...
    state.find(x => x.id === cid).cards.push({ id: crypto.randomUUID(), content: t, prio: prioClass, owner: user.name, ownerAvatar: user.avatar });
